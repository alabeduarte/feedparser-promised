...
describe('FeedparserPromised', () => {
  ...
  describe('.parse cp1251', () => {
    const aHost = faker.internet.url();
    const aPath = '/rss';
    const someUrl = `${aHost}${aPath}`;
    const rssFile = fs.readFileSync(`${__dirname}/feeds/rss2sample-cp1251.xml`)
    const expectedItems = [
      { title: 'Звездный город' },
      { title: 'The Engine That Does More' },
      { title: 'Astronauts Dirty Laundry' }
    ];
    ...
    describe('on success', () => {
      it('parses rss items', (done) => {
        nock(aHost).get(aPath).reply(200, rssFile, { 'content-type': 'application/rss+xml; charset=windows-1251' });
  ...
  describe('.parse gzipped', () => {
    const aHost = faker.internet.url();
    const aPath = '/rss';
    const someUrl = `${aHost}${aPath}`;
    const rssFile = fs.readFileSync(`${__dirname}/feeds/rss2sample.xml.gz`)
    const expectedItems = [
      { title: 'Star City' },
      { title: 'The Engine That Does More' },
      { title: 'Astronauts Dirty Laundry' }
    ...
    describe('on success', () => {
      it('parses rss items', (done) => {
        nock(aHost).get(aPath).reply(200, rssFile, { 'content-encoding': 'gzip' });
  ...
  describe('.parse cp1251 gzipped', () => {
    const aHost = faker.internet.url();
    const aPath = '/rss';
    const someUrl = `${aHost}${aPath}`;
    const rssFile = fs.readFileSync(`${__dirname}/feeds/rss2sample-cp1251.xml.gz`)
    const expectedItems = [
      { title: 'Звездный город' },
      { title: 'The Engine That Does More' },
      { title: 'Astronauts Dirty Laundry' }
